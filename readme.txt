...
9.git stash 将工作现场储存起来，如工作到一半还没提交，需要修改另一个bug，使用该命令可以将工作现场储存，以便修改紧急问题不影响。
10.git stash list 就是指工作区临时储存的现场
11.git stash apply恢复，但是恢复后，stash内容并不删除，需要使用12来删除
12.git stash drop来删除现场储存的东西；
13.git stash pop，恢复的同时把stash内容也删了：（即类似11 12 一起）
14.git cherry-pick <commit> 其中commit就指的那一串提交的id；
如：（9，10，11，12，13，14）用法场景，我在dev分支上开发新功能，该功能与老代码耦合性比较高牵动的地方比较多，不巧客户正式环境报了个紧急bug，那怎么办？现在的我代码和正式环境的代码不一致了，我现在不想注释新代码。
因此我们需要用到9来储存现场的代码，然后再master 分支上开一个新分支（master-01）来修改这个紧急bug，完了后合并到master 并更新到正式。好了紧急问题解决了，我现在要开始继续我的新功能开发了，因此首先把储存的代码恢复 用（11，12）或13
其次我dev的代码来自于master 既然master上有客户的紧急bug，同样dev也有，但是同样的bug我不想改两遍 ，因此git提供了14给我们 。最后我们只需要把master的bug修改记录 复制到dev即可。避免同样的问题改两边 